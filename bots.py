...
# list_scores = {0:[],1:[],2:[],3:[],4:[],5:[],6:[]}
# int_dice_remaining = 0
# int_unbanked = 0
class BaseBot(ABC):
    ...
    def play(cls, num_games=1):
        ...
        # list_scores[int_dice_remaining].append({'name': f"{int_dice_remaining}_{int_unbanked}" , 'value':(mega_total // num_games)})
...
if __name__ == "__main__":
    ...
    #     # print(i)
    #     int_dice_remaining = i
    #     for j in range(100,10000,50):
    #         print(i,j)
    #         int_unbanked = j
    #         Bank2650_Roll3_Bot.play(num_games)
    #         if len(list_scores[int_dice_remaining]) > 20:
    #             if min([i['value'] for i in list_scores[int_dice_remaining][-19:]]) >= list_scores[int_dice_remaining][-1]['value']:
    #                 break
    #
    #
    # max_score = {'name':None,'value':0}
    # for i in list_scores.keys():
    #     for j in list_scores[i]:
    #         if max_score['value'] < j['value']:
    #             max_score = j
    # print(max_score)
